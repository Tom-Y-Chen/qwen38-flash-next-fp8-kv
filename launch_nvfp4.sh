#!/usr/bin/env bash
# EXPERIMENTAL: launch Qwen3.8-Flash-Next with NVFP4 KV cache on a single
# DGX Spark (GB10 / sm_121).  NVFP4 is a memory win (packed fp4 + fp8 block
# scales, 0.5625 B/elem vs bf16 2.0 / fp8 1.0) -> the KV-token ceiling jumps
# ~3.56x vs bf16; expect roughly flat decode speed since QSA is sparse and not
# KV-bandwidth-bound.  Uses the Part-0 winner num_spec=2 for MTP.
set -euo pipefail

NUM_SPEC="${1:-2}"
NAME=qwen38-flash-nvfp4-${NUM_SPEC}
SNAP=/hf/hub/models--RadixArk--Qwen3.8-Flash-Next-NVFP4/snapshots/7b719225242aacd3dbd3f9407468c2ee9a9d2594
IMAGE=qwen38-flash-dgx-nvfp4
KVD=nvfp4
...
docker run -d --name "$NAME" --gpus all --ipc=host --shm-size 16g -p 18307:8000 \
  ...
  "$IMAGE" "$SNAP" --served-model-name qwen3.8-flash-next \
    ...
    --max-model-len 262144 --max-num-seqs 8 --gpu-memory-utilization 0.90 \
    --kv-cache-dtype "$KVD" \
...
echo "started $NAME KVD=$KVD num_spec=$NUM_SPEC GMU=0.90"
